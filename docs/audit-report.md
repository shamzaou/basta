...
| 24 | ⚪ | Frontend hygiene | Leftover `// <<<<<<< master` merge-marker comments, two definitions of `loadTournamentData` (last wins), `alert()`-driven UX, Russian comments. Cosmetic; left untouched so the team recognises its code. |
