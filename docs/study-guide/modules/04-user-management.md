...
| Avatar serving | `get_avatar_image` (FileResponse, default `static/frontend/assets/man.png`) | `userapp/views.py:900-936` |
| Friends | `get_all_users` (with `is_friend`), `get_friends`, `add_friend`, `remove_friend` | `userapp/views.py:1031-1149` |
...
| Export | `export_user_data` | `userapp/views.py:970-1029` |
...
| URLs | `/api/auth/register/ login/ logout/ profile/ settings/ match-history/ save-match/ match/create/ delete-account/ anonymize-account/ avatar/<id>/ export-data/ users/ friends/ friends/add|remove/<id>/ redirect_uri/ get-token/ token/ token/refresh/ verify-otp/` | `userapp/urls.py:12-51` |
| Frontend | `handleRegister` `script.js:414`, `handleLogin` `:256`, `handleLogout` `:378`, `loadProfileData` `:1090`, avatar upload (FileReader → base64) `:723-776`, settings edit buttons `:645-701`, friends `:1731-1945`, pie chart `:1948`, OAuth `initiate42OAuth` `:976`, `checkOAuthLogin` `:1019` | `static/frontend/js/script.js` |
...
| Login gate for tournament/game pages | `showPage` redirects logged-out users; Play/Tournament buttons check `isLoggedIn` | `script.js:13-21`, `:894-940` |
...
**🆕 Changed in Aug-2026 audit:** `verify_otp` now also returns `refresh_token` (`userapp/views.py:337`); `refreshAccessToken` calls the correct `/api/auth/token/refresh/` (`script.js:1468`) and `handleLogout()` on failure (`:1485`); an "Anonymize My Account" GDPR action was added (`:851`); the OTP login path is fixed (see 2FA module).
...
3. **How does the avatar upload work?** The browser reads the file as a data-URL (`script.js:776`), PUTs it as JSON; the view base64-decodes and saves `profile_pictures/user_<id>.<ext>` (`views.py:177-181`); `/api/auth/avatar/<id>/` streams it or the default (`:900-936`).
...
5. **How do friends work? Is it mutual?** Non-symmetric M2M (`models.py:35`): A adding B does not add A to B's list. "Find Users" shows `is_friend` per user (`views.py:1033-1059`).
